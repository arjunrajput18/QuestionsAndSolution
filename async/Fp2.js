...
// fakeFetch10("https://example.com/getImage", [200, 200]).then(
// ({status,data:{image:{link,title}}}) =>{
// if(status===200){
//   outputBox.innerHTML=`<img src=${link}/><div>title:${title}</div>`
// }
// }).catch(()=>{
//   outputBox.innerHTML=`  wait for someTime`
// }
// )

const showImg=async ()=>{
 try{
  const {status,data:{image:{link,title}}}=await fakeFetch10("https://example.com/getImage", [200, 200])
